...
def generate_all_number_using_python(num_decimals: int, period: int, seed: int = 25, nb_number_generate:int = 2000000):
    
    random.seed(seed)
    
    return [round(random.random(), num_decimals) for _ in range(0, nb_number_generate - (num_decimals - period), period)]
...
def test_our_generator(file_path: str, num_interval: int = 10):
    ...
    for num_decimals in range(3, 15):
...
def test_python_generator(num_interval: int = 10):
    ...
    #         data = generate_all_number_using_python(num_decimals = num_decimals, period = period)
...
if __name__ == "__main__":
    ...
    # test_python_generator(file_path = file_path)